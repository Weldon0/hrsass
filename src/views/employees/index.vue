  ...
  <div class="dashboard-container">
    <div class="app-container">
      <PageTools :has-before="true">
        ...
        <template #after>
          <el-button size="mini" type="warning" @click="$router.push(`/import?type=em`)">导入</el-button>
  ...
  methods: {
    ...
    formatJson(headers, rows) {
      ...
      return rows.map(item => {
        return Object.keys(headers).map(key => { // [['黑马', '791119'],['黑马', '791119'],['黑马', '791119']]
          //   key ==> 中文的key
          //  如何获取到中文key所对应的英文key
          ...
          if (enKey === 'formOfEmployment') {
            const pyxs = mapData.hireType.find(item => item.id === item[enKey])?.value
            return pyxs || '未知'
          }
          return item[enKey]
        })
    ...
    formatEmployment(row, column, cellValue, index) {
      const data = EmployeeEnum.hireType.find(item => item.id === cellValue)?.value
      return data || '未知'
    },
    // 获取列表数据
    async getList() {
